...
Scope: cli-root-yo MUST provide a reusable CLI kernel that downstream repositories use to build a unified command-line interface with consistent behavior, output style, help, and extension semantics.
cli-root-yo MUST be responsible for:
...
Non-Goals: cli-root-yo MUST NOT do any of the following:
...
STDERR MUST remain unused by cli-root-yo in normal operation.
...
This section defines the cli-root-yo library API, modules, and override rules.
3.1 Required external dependencies
cli-root-yo MUST depend on:
...
cli-root-yo MUST provide these modules and responsibilities:
...
cli-root-yo MUST expose these public entrypoints:


cli_root_yo.app.create_app(spec: CliSpec) -> typer.Typer
...
cli_root_yo.app.run(spec: CliSpec, argv: list[str] | None) -> int
...
cli_root_yo.registry.CommandRegistry
...
cli_root_yo.runtime.get_context() -> RuntimeContext
...
Depend on cli-root-yo via pip.


Provide its own console script entrypoint (Section 5.3) that calls cli_root_yo.app.run(spec, argv=None).
...
cli-root-yo MUST support two plugin sources:
...
Entry point group name is exactly: cli_root_yo.plugins
...
If spec.config is not null, cli-root-yo MUST register config group with these subcommands:
...
If spec.env is not null, cli-root-yo MUST register env group with these subcommands:
...
cli-root-yo MUST support Python >=3.10.


cli-root-yo MUST NOT use features requiring Python >=3.11.


5.2 Packaging format


Distribution name MUST be cli-root-yo.


Import package name MUST be cli_root_yo.
...
cli-root-yo MUST NOT install a user-facing console script by default.


Each downstream repository MUST provide its own console script entry point that invokes cli_root_yo.app.run() with its CliSpec.
...
cli-root-yo MUST use XDG directories on both macOS and Linux (Section 6.4).
...
cli-root-yo MUST define and downstream commands MUST use these primitives:
...
Debug mode is enabled if environment variable CLI_ROOT_YO_DEBUG is set to 1.
...
7.1 What logic MUST move to cli-root-yo
From zebra_day, the following MUST be extracted or reimplemented in cli-root-yo as shared behavior:
...
Create the new package cli-root-yo implementing Sections 2 through 6.
...
Update JSON output in zebra_day commands to use cli-root-yo JSON emitter to eliminate invalid JSON caused by Rich wrapping.
